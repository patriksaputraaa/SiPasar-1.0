...
class AdminController extends BaseController
{
    ...
    public function addgedung(Request $request)
    {
        ...
        if ($userGet == null) {
            ...
            if (empty($exist)) {
                DB::insert(
                    "INSERT INTO pasarlantai (id_pasar, lantai, nama_gedung, kode_svg)
            VALUES (?, ?, ?, ?)",
    ...
    public function addutama(Request $request)
    {
        ...
        if ($userGet == null) {
            ...
            if (empty($exist)) {
                DB::insert(
                    "INSERT INTO pasar (nama_pasar, kelas_pasar, alamat_pasar,
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, GeomFromText( CONCAT('POINT(', ?, ' ', ?, ')')))",
                    [
                        $nama_pasar, $kelas_pasar, $alamat_pasar, $kecamatan_pasar, $kodepos_pasar, $instagram_pasar, $deskripsi_pasar, $sejarah_pasar, $jumlah_pedagang, $jumlah_lantai, $is_aktif, $is_denah, $latitude, $longitude
                    ]
                );
            } else {
                return response()->json("sudah ada");
    ...
    public function editutama(Request $request)
    {
        ...
        if ($userGet == null) {
            ...
            DB::insert(
                "UPDATE pasar SET nama_pasar=?, kelas_pasar=?, alamat_pasar=?,
        kecamatan_pasar=?, kodepos=?, instagram=?, deskripsi_pasar=?,
        sejarah_pasar=?, jumlah_pedagang=?, jumlah_lantai=?, is_aktif=?, is_denah=?, lonlan=GeomFromText( CONCAT('POINT(', ?, ' ', ?, ')'))
        WHERE id_pasar=?",
                [
                    $nama_pasar, $kelas_pasar, $alamat_pasar, $kecamatan_pasar, $kodepos_pasar, $instagram_pasar, $deskripsi_pasar, $sejarah_pasar, $jumlah_pedagang, $jumlah_lantai, $is_aktif, $is_denah, $latitude, $longitude, $id_pasar
                ]
            );
